...
#endif
{
	...
	if ( name!=NULL ) {
		fprintf(ErrFile, "SetWordType %s%s[%zu] = {",
				name,
                suffix,
				NumWords(TokenNum-1)*sizeof(unsigned));
	}
	else {
		fprintf(ErrFile, "SetWordType zzerr%d[%zu] = {",
				esetnum,
				NumWords(TokenNum-1)*sizeof(unsigned));
	}
	while ( p < endp )
...
#endif
{
	...
	if ( name!=NULL ) {
		fprintf(Parser_h, "\tstatic SetWordType %s%s[%zu];\n", name, suffix,
				NumWords(TokenNum-1)*sizeof(unsigned));
		fprintf(Parser_c, "SetWordType %s::%s%s[%zu] = {",
				CurrentClassName,
				name,
				suffix,
				NumWords(TokenNum-1)*sizeof(unsigned));
		...
		fprintf(Parser_c, "SetWordType %s::err%d[%zu] = {",
				CurrentClassName,
				esetnum,
				NumWords(TokenNum-1)*sizeof(unsigned));
		fprintf(Parser_h, "\tstatic SetWordType err%d[%zu];\n", esetnum,
				NumWords(TokenNum-1)*sizeof(unsigned));
	}
...
#endif
{
  ...
  if ( FirstAction != NULL ) dumpAction(FirstAction,Parser_c, 0, -1, 0, 1);    /* MR11 MR15b */
	...
	/* Build constructors */
	fprintf(Parser_c, "\n%s::", CurrentClassName);
	fprintf(Parser_c,	"%s(ANTLRTokenBuffer *input) : %s(input,%d,%d,%d,%zu)\n",
						CurrentClassName,
...
#endif
	fprintf(ErrFile, "#define zzSET_SIZE %zu\n", NumWords(TokenNum-1)*sizeof(unsigned));
